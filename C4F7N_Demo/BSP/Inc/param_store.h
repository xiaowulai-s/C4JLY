 ...
 *
 * 扇区规划 (W25Q16: 512×4KB = 2MB):
 *   [0x00000] 第 0 扇区 (4KB)      : 标定 + 设置 (本模块, 整扇区擦写, 低频)
 *   [0x01000..0x1FF000] 扇区1~511  : 预留历史数据 (约2.08MB; 2字节/点→104万点,
 *                                      1s采样≈12天 / 60s采样≈2年, 环形磨损均衡)
 * 默认仅用到第0扇区; 修改即整扇区(4KB)擦除重写 (标定/设置为低频操作, 写寿命足够)。
 */
