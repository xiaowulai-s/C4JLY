...
| 电池电量（新增） | PB1(ADC1_IN9) 分压采样 → 电量%，节流刷新屏幕 | 构建通过 [Data-backed] |
...
| 电池电压 | PB1 (ADC1_IN9) | R22=100k + R23=24k 分压，14.8V/16.8V 电池 |
...
### CubeMX 配置（Project Manager → Code Generator）需确认

- ✅ **Generate peripheral initialization as a pair of .c/.h files per peripheral**
- ✅ **Keep user code when re-generating**（保护 USER CODE 段）
- ⚠️ **Delete previously generated files when not in use 保持【不勾选】**——否则 CubeMX 会把不认识的 `BSP/`、`Middleware/` 当作"不再使用"删除
...
| `Core/Inc/stm32f1xx_hal_conf.h` 的 `HAL_ADC_MODULE_ENABLED` 保持启用 | `ADC_HandleTypeDef` 未定义，battery.c / main.c 编译失败 |
