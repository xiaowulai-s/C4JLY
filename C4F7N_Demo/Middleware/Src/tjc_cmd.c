...
/* 更新所有页面的电量显示 (顶栏 chip x4 + 关于页电量行) */
void tjc_set_bat(int pct) {
  char cmd[40];
  const char *objs[] = { OBJ_BAT_CHIP, OBJ_BAT_CHIP_P1, OBJ_BAT_CHIP_P2, OBJ_BAT_CHIP_P3, OBJ_BAT_ROW };
  unsigned i;
  for (i = 0; i < sizeof(objs)/sizeof(objs[0]); i++) {
    HMI_CMD_SET_BAT(objs[i], pct);
    tjc_send_raw(cmd);
  }
}
